...
def aggregate_emb(root_path):
    total = torch.Tensor()
    for item in tqdm(os.listdir(f"{root_path}/save/recom_item_output/candidate_emb_3")):
        emb = torch.load(f"{root_path}/save/recom_item_output/candidate_emb_3/{item}")
        total = torch.cat((total, emb), dim=0)
    torch.save(total, f"{root_path}/save/recom_item_output/total_3.pt")
    return total
