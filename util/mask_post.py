...
def apply_mask(image, masks, labels, boxes, file_name, classes):
    ...
    image_only = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) # 누끼 딴 이미지를 만들기 위해 생성
    # width = image.shape[0]
    ...
    for n in range(masks.shape[0]):
        ...
        # 누끼 딴 이미지 생성
        image_only[:, :, 0][masks[n][0] <= 0.5] = 255
        image_only[:, :, 1][masks[n][0] <= 0.5] = 255
        image_only[:, :, 2][masks[n][0] <= 0.5] = 255
